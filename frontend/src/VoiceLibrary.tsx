...
export default function VoiceLibrary() {
  ...
  const waiting=!!sample && ['queued','running'].includes(sample.status);
  ...
  return <section className="library-page">
    ...
    {waiting&&<p role="status">{sample.status==='queued'?'Chờ worker CPU…':'Đang tạo mẫu giọng…'}</p>}
    {sample?.status==='failed'&&<p role="alert">{sample.message}</p>}
    {sample?.status==='complete'&&<audio key={sample.id} controls autoPlay src={'/api/library/samples/'+sample.id+'/audio'}/>}
    <p>{visible.length} / {voices.length} giọng</p>
    {visible.map(v=><article className="voice-card" key={v.id}><div><strong>{v.name}</strong><small>{v.id}</small><p>{v.description}</p><small>{[v.gender,v.region,v.style,...v.tags].filter(Boolean).join(' · ')}</small></div><button disabled={waiting} onClick={()=>listen(v.id)}>Nghe mẫu</button><button aria-label={'Yêu thích '+v.name} aria-pressed={v.favorite} disabled={saving} onClick={()=>save({...v,favorite:!v.favorite})}>{v.favorite?'★':'☆'}</button><button onClick={()=>setDraft({...v})}>Sửa</button></article>)}
